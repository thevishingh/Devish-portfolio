    ...
    <div className="relative pt-24 pb-20 overflow-hidden">
      ...
      <div className="absolute inset-0 overflow-hidden pointer-events-none">
        {[...Array(8)].map((_, i) => (
          <motion.div
            key={i}
            animate={{ y: [0, -15 - i * 4, 0], opacity: [0.15, 0.5, 0.15] }}
            transition={{
              duration: 3 + i * 0.7,
              repeat: Infinity,
              ease: "easeInOut",
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          ...
          <div>
            ...
            >
              From Intern to
              <br />
              <span className="text-[var(--accent-cyan)]">React Developer</span>
            </motion.h1>
            ...
            >
              A 10-month internship ignited my passion. 2+ years later, I&apos;m
              building production apps that serve thousands of users with clean,
            ...
            >
              {personalInfo.stats.map((stat, i) => (
                <div
                  key={i}
                  className="p-3 rounded-lg bg-[var(--bg-card)] border border-[var(--border-color)] text-center"
                >
                  <div className="font-mono text-xl font-bold text-[var(--accent-cyan)]">
                    {stat.value}
                  </div>
                  <div className="font-mono text-[10px] text-[var(--text-secondary)] uppercase tracking-wider mt-0.5">
                    {stat.label}
                  </div>
                </div>
              ))}
          ...
          >
            ...
            >
              ...
              <div className="relative w-72 h-80 sm:w-80 sm:h-96 rounded-2xl overflow-hidden border-2 border-[var(--accent-cyan)]/20 shadow-2xl shadow-[var(--accent-cyan)]/10 group">
                <img
                  src="./profile.png"
                  alt="Vishak Singh - React Developer"
                ...
                <div className="absolute bottom-4 left-4 right-4 p-3 rounded-lg bg-[var(--bg-primary)]/80 backdrop-blur-sm border border-[var(--accent-cyan)]/10">
                  <div className="font-mono text-[10px] text-[var(--accent-cyan)]/60 mb-1">
                    // current status
                  </div>
                  ...
                  <div className="font-mono text-xs text-[var(--text-primary)]">
                    const passion ={" "}
                    <span className="text-[var(--accent-green)]">true</span>;
                  </div>
                </div>
              </div>
...
function TimelineItem({ item, index }) {
  ...
  return (
    <div
      ref={ref}
      className="relative grid md:grid-cols-[1fr_60px_1fr] gap-0 group"
    >
      {/* Left content */}
      <div
        className={`${isLeft ? "md:block" : "md:block md:invisible"} hidden`}
      >
        {isLeft && (
          <motion.div
            initial={{ opacity: 0, x: -40 }}
          ...
          >
            <div className="flex items-center gap-2 mb-2 flex-wrap">
              <span className="font-mono text-xs text-[var(--accent-cyan)] bg-[var(--accent-cyan)]/5 px-2 py-0.5 rounded">
                {item.date}
              </span>
              <span className="font-mono text-xs text-[var(--text-secondary)] capitalize px-2 py-0.5 rounded bg-[var(--bg-primary)]">
                {item.type}
              </span>
            </div>
            <h3 className="font-mono font-bold text-lg text-[var(--text-primary)] mb-1">
              {item.title}
            </h3>
            <p className="text-sm text-[var(--accent-cyan)]/70 font-mono mb-3">
              {item.organization}
            </p>
            ...
            <div className="flex flex-wrap gap-2">
              {item.highlights.map((h, i) => (
                <span
                  key={i}
                  className="text-xs font-mono px-2 py-1 rounded bg-[var(--accent-cyan)]/5 text-[var(--accent-cyan)]/80 border border-[var(--accent-cyan)]/10"
                >
                  {h}
                </span>
              ))}
      ...
      <div className="md:hidden flex gap-4">
        ...
        >
          ...
          <h3 className="font-mono font-bold text-base text-[var(--text-primary)] mb-1">
            {item.title}
          </h3>
          <p className="text-sm text-[var(--accent-cyan)]/70 font-mono mb-2">
            {item.organization}
          </p>
          <p className="text-sm text-[var(--text-secondary)] leading-relaxed mb-3">
            {item.description}
          ...
          <div className="lg:col-span-2 about-stats-container">
            <div className="grid grid-cols-2 gap-4">
              {personalInfo.stats.map((stat, i) => (
                <div
                  key={i}
                  className="about-stat p-5 rounded-xl bg-[var(--bg-card)] border border-[var(--border-color)] hover:border-[var(--accent-cyan)]/20 transition-all duration-500 group"
                >
